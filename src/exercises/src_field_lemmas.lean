...
lemma zero_add (x : R) : 0 + x = x := (comm_group.add_comm x 0) ▸ (add_zero x)
lemma add_neg (x : R) : x + (-x) = 0 := comm_group.add_inv x
lemma neg_add (x : R) : (-x) + x = 0 := (comm_group.add_comm x (-x)) ▸ (add_neg x)
...
lemma one_mul (x : R) : 1 * x = x := (myfield.mul_comm x 1) ▸ (mul_one x)
lemma mul_inv (x : R) : x ≠ 0 → x * x⁻¹  = 1 := myfield.mul_inv x
lemma inv_mul (x : R) : x ≠ 0 → x⁻¹ * x = 1 := (myfield.mul_comm x x⁻¹) ▸ (mul_inv x)
...
begin
  rw ←(add_zero (x*0)),
  conv { to_lhs, congr, skip, rw ←(add_neg (x*0)) },
  rw ←add_assoc,
  conv in (x*0 + x*0) { rw [←mul_add, zero_add], },
  rw add_neg,
end
